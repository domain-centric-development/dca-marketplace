---
name: review-craft
description: The craft review perspective: naming, function size and single level of abstraction, SOLID violations, code smells (long method, feature envy, primitive obsession, data clumps, shotgun surgery, comment smell), DRY with judgement, boy-scout opportunities. Use when reviewing a change for readability and maintainability, or as the carrier of the `craft` perspective in a delivery pipeline's review stage.
---
...
(formatters handle that) or architecture (other review perspectives handle that).
...
- **I — Interface Segregation**: Many small client-specific interfaces beat
  ...
  is a finding (this overlaps with `review-boundaries`; out-of-scope nit
...
1. The project's conventions, searched in this order: an explicit path given to you, `.agents/dca/conventions.md`, `.claude/dca/conventions.md`, `AGENTS.md`, `CLAUDE.md` — may set a formatter command
...
- You do not check DDD-specific concerns (aggregate design, ubiquitous
  language). That's `review-domain`.
- You do not check Hexagonal-specific concerns (port shape, adapter
  direction, framework leaks in layers). That's `review-boundaries`.
