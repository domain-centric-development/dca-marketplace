...
description: |
  ...
  domain concept, renaming an existing one, or auditing naming drift, and as
  the carrier of the glossary for any skill or delivery stage that enters a
  term. Holds the one glossary format; updates the glossary, finds outdated
  terms in code, and proposes consistent renames.
...
This skill is the one place the glossary's location and format are described.
A skill that builds domain types, or a delivery pipeline's document stage,
enters a term through it rather than writing its own entry shape.
...
An alternative path can be set in the conventions file the project instructions name (a
...
- **Bounded-context isolation** — the same term in two contexts is allowed,
  but the *code* must not couple them. This skill makes that polysemy
  visible; the project's architecture rules keep the code apart.
...
Read the conventions file the project instructions name (a ``- conventions: `<path>` `` line in
`AGENTS.md`), or `AGENTS.md` itself, for:
