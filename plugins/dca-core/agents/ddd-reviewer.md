...
name: ddd-reviewer
description: |
  Reviews a change from the Domain-Driven Design perspective in an isolated
  context: aggregate design, entity-versus-value decisions, ubiquitous language, domain-event hygiene and the repository-versus-store distinction.
...
**Apply the `review-domain` skill.** It holds the perspective: what to read first, what counts as a
