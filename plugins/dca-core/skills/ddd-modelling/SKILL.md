...
from a DDD perspective, apply the `review-domain` skill instead.)

## How you read the project (mandatory first step)

Before writing any code, gather context:

1. **Read the project's conventions, searched in this order: an explicit path given to you, `.agents/dca/conventions.md`, `.claude/dca/conventions.md`, `AGENTS.md`, `CLAUDE.md`.** Take from it:
...
- **`review-domain`** is your review-only counterpart. After building, the
  user may invoke `review-domain` (often in parallel with `review-boundaries`
  and `review-craft`) for a fresh perspective on what you wrote.
...
- **You do not review existing code semantically.** That's `review-domain`.
