...
5. **Assert on the contract, not internals** — check the `Result` fields and observable side effects (e.g. `fakeRepo.findById(...)` present/absent). Never assert on private methods or fields ([common mistakes](/book/12-testing-strategy/common-mistakes.md)).
...
- Use hand-written fakes over Mockito to match the reference stack ([test doubles](/book/12-testing-strategy/test-doubles.md)).
...
- Book: [Application-layer testing](/book/12-testing-strategy/application-layer-testing.md) · [Test doubles](/book/12-testing-strategy/test-doubles.md) · [Testing pyramid](/book/12-testing-strategy/testing-pyramid.md)
