...
evidence: [/guide/rules.md, /rule/tactical/dca-tac-016.md, /rule/tactical/dca-tac-013.md, /rule/tactical/dca-tac-014.md, /rule/tactical/dca-tac-015.md, /rule/tactical/dca-tac-017.md, /rule/naming/dca-nam-004.md, /rule/hexagonal/dca-hex-003.md]
---
...
4. **Implement in an outgoing adapter** — e.g. `InMemory{Name}Repository` in `adapter/outgoing/`. The implementation may use framework/persistence types; the interface must not. Every adapter hands out copies through `reconstitute`, never the stored instance ([Layer rules](/guide/rules.md)).
...
6. **Use explicit `save()`** — persistence-oriented style, not a live collection illusion ([Layer rules](/guide/rules.md)). The use case calls `save` after mutating, then `publishAndClearEvents` — one call, not a loop over `publish(event)` ([Publishing events one by one](/pitfall/publish-instead-of-publish-and-clear.md)).
...
- Guide: [Layer rules](/guide/rules.md) · [Deviations from the literature](/guide/readme/deviations-from-the-literature.md) · [Layer elements](/guide/elements.md) · [Port placement](/guide/quick-reference/port-placement.md)
