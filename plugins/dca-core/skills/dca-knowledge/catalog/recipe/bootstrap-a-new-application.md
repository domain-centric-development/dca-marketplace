...
0. **Describe it, then generate it** — before any code, the product, the technical decisions and the designed cut ([Three parts](/guide/factory/three-parts.md), [The project description](/guide/factory/the-project-description.md)); then the skeleton from the stack's own generator, so the versions are the generator's, never recalled; then a repository. A formatter is run once over the whole code base when it is added, and a product with pages gets a browser runner before its first page ([Add a browser runner](/recipe/add-a-browser-runner.md)).
...
- Guide: [Java package structure](/guide/package-structure.md) · [ArchUnit adoption path](/guide/archunit-governance/adoption-path-tiers.md) · [Port placement](/guide/quick-reference/port-placement.md) · [Shared kernel](/guide/spring-modulith/shared-kernel-in-spring-modulith.md) · [Core rule categories](/guide/archunit-governance/core-rule-categories.md) · [Context-specific rule sets](/guide/archunit-governance/context-specific-rule-sets.md) · [Layer elements](/guide/elements.md)
- One capability later: [Add a browser runner](/recipe/add-a-browser-runner.md)
