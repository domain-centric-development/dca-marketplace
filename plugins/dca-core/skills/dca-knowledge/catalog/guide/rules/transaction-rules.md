...
- **A remote effect inside the use case comes after the aggregate has agreed to it.** When the effect cannot wait for the commit — a payment intent, a reservation, a label — load the aggregate first and ask it: give it a method that asserts the preconditions of the mutating one and changes nothing (`assertReadyFor…()`). Otherwise the effect exists at the other system for an operation this system then refuses, and a rollback does not reach it
- **Say what closes the remaining window.** The check cannot be atomic with the write, so either compensate on failure (call the port's release operation for the reference just made, re-throw the original failure, log a refused compensation rather than raising it) or make the effect idempotent under a key derived from the aggregate's identity. Compensation uses the port as it stands; idempotency changes its contract. Whichever is chosen, record that a process dying in between orphans the effect and only reconciliation finds it
