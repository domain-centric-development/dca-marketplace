---
type: Guide
title: Deployment Patterns
source: guide
resource: implementing-domain-centric-architecture/deployment-patterns.md
tags: [guide, guide]
---

*Extending Domain-Centric Architecture with Deployment Strategies*

> **📘 Prerequisites:** This document extends [Domain-Centric Architecture](/guide/readme.md). Read the main document first for core architectural patterns, layers, and rules.

## Sections

- [Introduction](/guide/deployment-patterns/introduction.md)
- [Self-Contained Systems (SCS)](/guide/deployment-patterns/self-contained-systems-scs.md)
- [Service Decomposition](/guide/deployment-patterns/service-decomposition.md)
- [Deployment Pattern Comparison](/guide/deployment-patterns/deployment-pattern-comparison.md)
- [Migration Path](/guide/deployment-patterns/migration-path.md)
- [Summary](/guide/deployment-patterns/summary.md)
