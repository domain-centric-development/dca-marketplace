...
- [Transactional use cases must not call remote-capable output ports](transactional-use-cases-must-not-call-remote-capable-output-ports.md) — A @Transactional use case holds a database connection for its whole run. Calling an output port that may leave the pr...
...
- [Use cases that publish domain events must run inside a transaction boundary](use-cases-that-publish-domain-events-must-run-inside-a-transaction-boundary.md) — Integration events are relayed after commit (@TransactionalEventListener, @ApplicationModuleListener) and their publi...
