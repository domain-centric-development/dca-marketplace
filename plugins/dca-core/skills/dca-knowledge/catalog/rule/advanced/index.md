...
- [Aggregates, entities and value objects must not use domain services](dca-adv-019.md) — A domain service exists for logic that spans several aggregates, so the application calls it; a model type that reach...
- [Domain service operations take an aggregate or an entity](dca-adv-020.md) — A domain service works on the model itself; an operation that only takes extracted values moves the decision out of t...
