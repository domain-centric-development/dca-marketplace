...
type: Rule
id: DCA-ADV-004
title: Domain events must not carry prohibited framework metadata
rule: "Domain objects carry no metadata for container management, persistence or transaction coordination."
constraint: Domain events must not carry prohibited framework metadata.
selects: "Non-interface domain events in domain packages. Metadata ownership is exclusive: events, services, factories, specifications, then domain.model types."
...
**Selection.** Non-interface domain events in domain namespaces. Exclusive ownership: events, services, factories, specifications, then domain-model types.
