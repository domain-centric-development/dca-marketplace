...
- [Converters must reside in portadapter package](converters-must-reside-in-portadapter-package.md) — Converters/Mappers translate between layers and should be in adapters.
...
- [DTOs must reside in portadapter package (not in domain or application)](dtos-must-reside-in-portadapter-package-not-in-domain-or-application.md) — DTOs are adapter concerns (presentation or external API) - not in domain or application.
