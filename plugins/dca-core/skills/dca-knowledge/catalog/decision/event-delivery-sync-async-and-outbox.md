...
- Related: [Note: domain vs integration events in an outbox](/note/outbox-domain-vs-integration-events.md) · [Pitfall: an event listener performs the external effect itself](/pitfall/event-listener-performs-the-external-effect.md) · [Where a multi-step process lives](/decision/coordinating-a-multi-step-process.md)
