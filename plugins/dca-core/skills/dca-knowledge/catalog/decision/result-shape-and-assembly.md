...
- Rules: [Results must not expose aggregate roots or entities](/rule/usecase/use-case-result-models-must-not-expose-aggregate-roots-or-entities.md) · [Incoming adapters must not depend on domain services](/rule/hexagonal/incoming-adapters-must-not-depend-on-domain-services.md) · [Results must end with `Result` and reside in the application package](/rule/usecase/use-case-result-models-must-end-with-result-and-reside-in-application-package.md) · [Results should be immutable](/rule/usecase/use-case-result-models-should-be-immutable-final-or-records.md)
...
- Related decisions: [Read model or domain query](/decision/read-model-vs-domain-query.md) · [Where does the logic live](/decision/where-does-the-logic-live.md)
