...
- Identity is an **output-port abstraction**, not a domain type. The current caller is reached through [IdentityProvider](/marker/port-out/identityprovider.md) — an `OutputPort` whose implementation lives in an adapter (JWT, session, API key). An aggregate that needs a `User`/roles either imports that infrastructure concern or duplicates a shadow identity model in the domain.
