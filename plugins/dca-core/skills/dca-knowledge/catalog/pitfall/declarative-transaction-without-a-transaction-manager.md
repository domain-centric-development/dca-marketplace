...
2. A `PlatformTransactionManager` bean. Until a database arrives, a small no-op subclass of `AbstractPlatformTransactionManager` **written in the project**, in a file whose name and comment say what replaces it. Deliberately not a library class: a published no-op manager survives into production unnoticed.
   ...
   **Retire it the moment a datasource exists** — including one that only holds the event publication registry, and including a profile that keeps in-memory repositories beside it. A no-op manager that outlives the "no database" state is worse than none at all: the registry writes its rows under the publishing transaction, so with nothing to roll back they commit on their own. The unit of work fails, the row stays, and the next recovery run delivers an event for work that never happened. Delivery in the same run looks correct throughout, because an after-commit listener is never handed a rolled-back publication — which is why this survives a test suite that only checks what listeners received. Check the registry's rows, not the listener, and let the framework's manager stand as soon as there is a resource for it to manage.
