...
Use this guide to recommend rule sets — when `dca-init` installs the rules, and when `dca-add rules` adds a set later. A recommendation is a
...
| **Brownfield retrofit** (introducing DCA into legacy code) | `cycles,hexagonal,naming,usecase` — add `tactical` and `errors` once classes implement the markers; freeze (`dca-add freeze`) or warn on the rest meanwhile |
...
- **`dca-archunit-spring-modulith`**: `spring-modulith-starter-test` is not on the class path — the
  base class does not load.
