...
- [ ] **Anti-pattern flag — Execution semantics as a port:** a transaction boundary or unit of work declared as an
      `OutputPort`. It is how the application runs, not a capability it lacks — `TransactionBoundary` (C#:
      `ITransactionBoundary`) is deliberately not a port and is not `OutputPort`'s subtype
...
### Declared relationships against the designed map

The strategic declarations on the context root (`@Upstream`, `@ExternalUpstream`, `@Partnership` on
`package-info.java`; C#: `[Upstream]`, `[ExternalUpstream]`, `[Partnership]` on the `XContext` marker class) are
the source for which relationships exist; the `contextmap` rule set (`DCA-MAP-*`) fails the build when a
declaration and the real dependencies disagree. What no rule sees is the designed map:

- [ ] Every declared relationship is on the designed map, and every relationship on the designed map is declared
      (or `PLANNED`) — a difference is a finding: designed and not built, or built without being designed
- [ ] The declared `translation` matches the map's translation (`ANTI_CORRUPTION_LAYER` ↔ an ACL, `CONFORMIST` ↔
      Conformist) and the declared `via` its channel — `API` for an Open Host Service or Customer/Supplier over the
      upstream's `api/`, `EVENTS` for a Published Language over its `events/`
- [ ] A declaration's `rationale` and the map's reason say the same thing; a declaration without a rationale where
      the map has one is a nit
- [ ] The rendered map (`ContextMapRenderer`, `docs/architecture/context-map.md` by default) is regenerated by its
      test, never hand-edited; a hand edit is a finding
- [ ] The map's subdomain type agrees with the rule sets the project runs (`dca-archunit.properties`) and with its
      pattern-selection decision: a Core context held only to the structural baseline, or a Supporting context held
      to the full tactical set, is a question for that decision, not a code fix
